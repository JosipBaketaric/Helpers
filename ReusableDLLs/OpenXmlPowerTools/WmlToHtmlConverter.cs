...
namespace OpenXmlPowerTools
{
    ...
    public static class WmlToHtmlConverter
    {
            ...
            decimal currentMarginLeft)
        {
            ...
            //josip
            if(element.Name.LocalName == W.alternateContent)
            {
                return ProcessParagraph(wordDoc, settings, element, suppressTrailingWhiteSpace, currentMarginLeft);
            }

            //josip
            if (element.Name.LocalName == W.fallback)
            {
                var txbxContentElement = element.Descendants(W.txbxContent).FirstOrDefault();
                if (txbxContentElement == null)
                    return null;

                txbxContentElement.Name = W.r;
                return ProcessParagraph(wordDoc, settings, txbxContentElement, suppressTrailingWhiteSpace, currentMarginLeft); 
            }
        ...
        private static object ConvertRun(WordprocessingDocument wordDoc, WmlToHtmlConverterSettings settings, XElement run)
        {
            ...
            if (rPr.Element(W.vertAlign) != null)
            {
                ...
                {
                    /*
                     * If <sub> or <sup> element appears then Xelement will consider them as new nodes
                     * hack: first send blank XText so it won't create new nodes and html won't append new lines
                     * */
